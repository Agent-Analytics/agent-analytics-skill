...
- For any live Agent Analytics task, always use the official CLI through `npx @agent-analytics/cli@0.5.12 <command>`.
...
- If a task needs multiple steps, compose the answer from multiple `npx @agent-analytics/cli@0.5.12 ...` commands instead of switching transports.
- If the CLI hits a limitation, stay on the CLI path, explain the limitation, and prefer `npx @agent-analytics/cli@0.5.12 feedback` over inventing a non-CLI workaround.
...
- OpenClaw can launch the official CLI with `npx @agent-analytics/cli@0.5.12`.
...
- Agents should still use the pinned `npx @agent-analytics/cli@0.5.12 ...` form instead of bypassing the CLI.
...
- Use only `npx @agent-analytics/cli@0.5.12 ...` for live queries unless the user explicitly requests API, MCP, or a local binary.
...
- If the task requires manual aggregation across projects, do that aggregation after collecting the data via repeated `npx @agent-analytics/cli@0.5.12 ...` calls.
- Validate project names before `create`: `^[a-zA-Z0-9._-]{1,64}$`

## First-time setup

```bash
npx @agent-analytics/cli@0.5.12 login --detached
npx @agent-analytics/cli@0.5.12 create my-site --domain https://mysite.com
npx @agent-analytics/cli@0.5.12 events my-site --days 7 --limit 20
```

For Paperclip, OpenClaw, and other issue-based runtimes, `login --detached` is the preferred first step. Send the approval URL to the user, wait for them to sign in with Google or GitHub, and expect a finish code reply. Then complete login and continue with project setup.
...
```bash
npx @agent-analytics/cli@0.5.12 login --detached
npx @agent-analytics/cli@0.5.12 login --token aak_YOUR_API_KEY
```
...
1. run `npx @agent-analytics/cli@0.5.12 login --detached`
...
For Paperclip company tasks, use this same detached loop even if the underlying runtime technically has browser automation available. The important behavior is that the task posts an approval URL, waits for the user's finish code, and then continues.
...
npx @agent-analytics/cli@0.5.12 all-sites --period 7d
npx @agent-analytics/cli@0.5.12 stats my-site --days 7
npx @agent-analytics/cli@0.5.12 insights my-site --period 7d
npx @agent-analytics/cli@0.5.12 events my-site --days 7 --limit 20
npx @agent-analytics/cli@0.5.12 breakdown my-site --property path --event page_view --days 7 --limit 10
npx @agent-analytics/cli@0.5.12 paths my-site --goal signup --since 30d --max-steps 5
npx @agent-analytics/cli@0.5.12 funnel my-site --steps "page_view,signup,purchase"
npx @agent-analytics/cli@0.5.12 retention my-site --period week --cohorts 8
npx @agent-analytics/cli@0.5.12 experiments list my-site
npx @agent-analytics/cli@0.5.12 update my-site --origins 'https://mysite.com,http://lvh.me:3101'
```

If a task needs something outside these common flows, use `npx @agent-analytics/cli@0.5.12 --help` first.
...
1. Run `npx @agent-analytics/cli@0.5.12 paths <project> --goal <event> --since 30d --max-steps 5`
...
1. Run `npx @agent-analytics/cli@0.5.12 projects`
2. For each project, run:

```bash
npx @agent-analytics/cli@0.5.12 query my-site --metrics session_count --days 2
```
...
Use `npx @agent-analytics/cli@0.5.12 feedback` when Agent Analytics was confusing, a task took too long, the workflow could be improved, or the agent had to do manual calculations or analysis that Agent Analytics should have handled.
...
```bash
npx @agent-analytics/cli@0.5.12 feedback --message "The agent had to calculate funnel drop-off manually" --project my-site --command "npx @agent-analytics/cli@0.5.12 funnel my-site --steps page_view,signup,purchase"
```
...
1. Run `npx @agent-analytics/cli@0.5.12 create my-site --domain https://mysite.com`
2. Copy the returned snippet into the page before `</body>`
3. Deploy
4. Verify with `npx @agent-analytics/cli@0.5.12 events my-site --days 7 --limit 20`
...
`npx @agent-analytics/cli@0.5.12 query` exists for advanced reporting, but it should be used carefully because `--filter` accepts JSON.

- Use fixed commands first.
- If `query` is necessary, check `npx @agent-analytics/cli@0.5.12 --help` first.
- Do not pass raw user text directly into `--filter`.
- The only valid CLI shape is `npx @agent-analytics/cli@0.5.12 query <project> ...`. Do not use `--project`.
...
2. Run `npx @agent-analytics/cli@0.5.12 properties <project>` to inspect event names and property keys first.
3. Use `npx @agent-analytics/cli@0.5.12 query <project> --filter ...` for property-filtered counts.
4. Use `npx @agent-analytics/cli@0.5.12 events <project>` only to validate ambiguous payloads or missing properties.
5. Use `npx @agent-analytics/cli@0.5.12 feedback` if the requested slice depends on unsupported grouping or derived reporting.
...
```bash
npx @agent-analytics/cli@0.5.12 properties my-site
npx @agent-analytics/cli@0.5.12 query my-site --metrics event_count --filter '[{"field":"event","op":"eq","value":"page_view"},{"field":"properties.first_utm_source","op":"eq","value":"reddit"}]' --days 30
```
...
```bash
npx @agent-analytics/cli@0.5.12 experiments list my-site
npx @agent-analytics/cli@0.5.12 experiments create my-site --name signup_cta --variants control,new_cta --goal signup
```
