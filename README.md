...
npx --yes @agent-analytics/cli@0.5.24 events <project> --event <first_useful_event> --days 7 --limit 20
```
...
The `create` command returns the exact tracking snippet for the project. Agents should add that snippet before `</body>`, but the snippet is the start of instrumentation, not the full instrumentation plan. After installing it, agents should review the local product flow, choose the smallest meaningful event set tied to the user's goals, explain what each event enables, and avoid generic tracking.

Agents should map event needs to tracker.js capabilities instead of inventing generic instrumentation: use `data-aa-event` for named click intent, `data-aa-impression` for meaningful section exposure, `window.aa.track(...)` for computed client state, and server-side tracking for durable outcomes such as completed signup, checkout, install, activation, or retained use. Relevant event candidates include named CTAs, signup intent, pricing interactions, checkout steps, install/setup progress, activation milestones, and durable server-side outcomes. Optional opt-ins such as scroll depth, forms, downloads, vitals, errors, performance, and SPA tracking should be enabled only when they unlock a concrete growth decision. Do not add custom duplicates for automatic tracker signals like `page_view`, path, referrer, UTMs, device/browser fields, country, session IDs, session count, days since first visit, or first-touch attribution.
